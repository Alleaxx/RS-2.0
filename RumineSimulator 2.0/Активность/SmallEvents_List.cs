...
namespace RumineSimulator_2._0
{
    static class SmallEvents_List
    {
        ...
        public static SmallEvent Ban_Random()
        {
            ...
            Event.EventAdd1_BasicInfo(new Event_Creator(CreatorType.User, user_init.nick), $"Модератор {user_init.nick} заметил неподобающее поведение одного из участников форума и пытается его пресечь!");
            ...
            Event.EventAdd2_Mods(random.Next(1, 4), 1, 0, 0);
            //Воплощение события
            bool banned = false;
            int chanse = 0;
            //Расчет шансов
        ...
        public static SmallEvent FailEvent()
        {
            ...
            Event.EventAdd2_Mods(0, 0, 0, 0);
            Event.EventAdd3_Participants(UserList.Users[random.Next(UserList.Users.Count)], Roles_moderator[random.Next(Roles_moderator.Count)]);
            Event.EventAddEnd_InterfaceInfo();
            return Event;
        }
    }
