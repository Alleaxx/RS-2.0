...
namespace RumineSimulator_2._0
{
    class Group
    {
        ...
        static public void GroupsCreation()
        {
            //Посетители
        ...
        static public Group ReturnRndGroup()
        {
        ...
        static public Group ReturnUserGroup(User user)
        {
            ...
            if (user.traits.Contains(Trait.SearchTrait(TraitsType.Wpower)))
            {
                return available_groups[0];
            }
            //Аскет - выдается худшая группа из доступных
            if (user.traits.Contains(Trait.SearchTrait(TraitsType.buddist)))
            {
                return available_groups[available_groups.Count - 1];
            }
            //Любитель новостей - выдается журналист или мб
            if (user.traits.Contains(Trait.SearchTrait(TraitsType.newslover)))
            {
                foreach (Group group in available_groups)
                {
                    if (group.Journ)
            ...
            if (user.traits.Contains(Trait.SearchTrait(TraitsType.vilka)))
            {
                foreach (Group group in available_groups)
                {
                    if (group.Mod_bezdn)
